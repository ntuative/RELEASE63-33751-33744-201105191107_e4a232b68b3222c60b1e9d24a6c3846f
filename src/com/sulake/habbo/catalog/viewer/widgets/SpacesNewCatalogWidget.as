package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class SpacesNewCatalogWidget extends ItemGridCatalogWidget implements IDisposable, ICatalogWidget, IItemGrid
   {
      ...
      private var var_422:ISelectorWindow;
      ...
      public function SpacesNewCatalogWidget(param1:IWindowContainer)
      {
         ...
         super(param1);
         this.var_422 = _window.findChildByName("groups") as ISelectorWindow;
         if(this.var_422)
         {
            _loc2_ = 0;
            while(_loc2_ < this.var_422.numSelectables)
            {
               _loc3_ = this.var_422.getSelectableAt(_loc2_);
               if(_loc3_ is ISelectableWindow)
               {
                  _loc3_.addEventListener(WindowEvent.const_49,this.onSelectGroup);
               }
               _loc2_++;
            }
         }
      ...
      override public function init() : Boolean
      {
         ...
         this.switchCategory(this.var_106[this.var_532]);
      ...
      {
         var _loc2_:int = this.var_646[this.var_532];
         var _loc3_:Offer = this.var_206[this.var_532][_loc2_];
      ...
      public function selectIndex(param1:int) : void
      {
         ...
         {
            _loc2_ = this.var_206[this.var_532][param1];
      ...
      override public function select(param1:IGridItem) : void
      {
         ...
         this.var_646[this.var_532] = (this.var_206[this.var_532] as Array).indexOf(_loc2_);
         this.updateRoomPreview();
      }
      
      private function updateRoomPreview() : void
      {
         var _loc1_:int = this.var_646[0];
         var _loc2_:int = this.var_646[1];
         var _loc3_:int = this.var_646[2];
         var _loc4_:Offer = this.var_206[0][_loc1_];
         var _loc5_:Offer = this.var_206[1][_loc2_];
         var _loc6_:Offer = this.var_206[2][_loc3_];
      ...
      private function createOfferGroups() : Boolean
      {
         ...
         {
            if(!(_loc1_.pricingModel == Offer.const_411 || _loc1_.pricingModel == Offer.const_418))
            {
               continue;
            }
            ...
            if(!(_loc2_.productType == ProductTypeEnum.const_64 || _loc2_.productType == ProductTypeEnum.const_83))
            {
               continue;
            }
            ...
            while(this.var_206.length < this._groupNames.length)
            {
               this.var_206.push(new Array());
            }
            switch(_loc4_)
            {
               case "floor":
                  (this.var_206[_loc5_] as Array).push(_loc1_);
                  break;
               case "wallpaper":
                  (this.var_206[_loc5_] as Array).push(_loc1_);
                  break;
      ...
      private function onSelectGroup(param1:WindowEvent) : void
      {
         ...
         if(_loc2_)
         {
            _loc3_ = this.var_422.getSelectableIndex(_loc2_);
      ...
      private function switchCategory(param1:String) : void
      {
         ...
         this.var_422.setSelected(this.var_422.getSelectableByName(param1));
         var _loc2_:int = -1;
         switch(param1)
         {
            case "group.walls":
         ...
         if(_loc2_ > -1)
         {
            ...
            page.replaceOffers(this.var_206[this.var_532],false);
            resetTimer();
            populateItemGrid();
            ...
            _loc3_ = this.var_646[this.var_532];
