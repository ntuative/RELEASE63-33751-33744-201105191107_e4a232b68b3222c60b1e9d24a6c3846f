package com.sulake.habbo.sound
{
   ...
   public class HabboSoundManagerFlash10 extends Component implements IHabboSoundManager, IUpdateReceiver
   {
      ...
      private static const const_1379:int = 60;
      
      private static const const_1380:int = 25165823;
      
      private static const const_1378:int = 16777215;
      ...
      private var var_905:Number = 1.0;
      ...
      public function HabboSoundManagerFlash10(param1:IContext, param2:uint = 0, param3:IAssetLibrary = null, param4:Boolean = true)
      {
         ...
         events.addEventListener(TraxSongLoadEvent.const_941,this.onTraxLoadComplete);
      ...
      private function loadSample(param1:int) : void
      {
         ...
         _loc4_.load(_loc3_);
         this.var_594.add(_loc4_,param1);
      }
      ...
      private function addTraxSongForDownload(param1:int, param2:String) : IHabboSound
      {
         ...
         if(!_loc3_.ready)
         {
            this.var_1141.add(param1,_loc3_);
         }
         return _loc3_;
      }
      ...
      private function createTraxInstance(param1:int, param2:String, param3:Boolean = true) : TraxSequencer
      {
         ...
         var _loc5_:TraxSequencer = new TraxSequencer(param1,_loc4_,this.var_250,events);
         _loc5_.volume = this.var_905;
      ...
      private function validateSampleAvailability(param1:TraxSequencer, param2:Boolean) : void
      {
         ...
         {
            if(this.var_250.getValue(int(_loc4_[_loc6_])) == null)
            {
               if(param2)
               {
      ...
      private function onHabboConfigurationInit(param1:IID = null, param2:IUnknown = null) : void
      {
         if(param2 != null)
         {
            ...
            if(this.var_195.getBoolean("trax.player.sample.memory.purge.enabled",false))
            {
               this.var_2300 = true;
            }
         }
      ...
      {
         if(this.var_195 == null || this._connection == null || this._roomEngine == null || this.var_223 != null)
         {
            return;
         }
         this.var_223 = new HabboMusicController(this,events,this._roomEngine.events,this._connection);
      }
      ...
      private function onTraxLoadComplete(param1:Event) : void
      {
         ...
         this.var_223.onSongLoaded(_loc2_.id);
      }
      
      private function ioErrorHandler(param1:Event) : void
      {
         events.dispatchEvent(new TraxSongLoadEvent(TraxSongLoadEvent.const_1764,this.var_736));
         this.var_736 = -1;
         this.var_737 = null;
      }
      ...
      private function storeVolumeSetting() : void
      {
         ...
         {
            this._connection.send(new SetSoundSettingsComposer(int(this.var_905 * 100)));
         }
      }
      ...
      {
         this.var_905 = param1;
         this.var_223.updateVolume(this.var_905);
      }
      ...
      private function processLoadedSample(param1:Sound) : void
      {
         ...
         var _loc4_:* = null;
         if(this.var_594.getValue(param1) != null)
         {
            _loc2_ = this.var_594.remove(param1);
            if(this.var_250.getValue(_loc2_) == null)
            {
               this.var_1407.clear();
               _loc3_ = param1.length;
               param1.extract(this.var_1407,int(_loc3_ * 44.1));
               _loc4_ = new TraxSample(this.var_1407,_loc2_,TraxSample.const_1685,TraxSample.const_1861);
               this.var_250.add(_loc2_,_loc4_);
            }
         }
      }
      ...
      private function processLoadedSamples() : void
      {
         ...
         if(this.var_1140.length > 0)
         {
            _loc1_ = getTimer();
            _loc2_ = _loc1_;
            while(_loc2_ - _loc1_ < const_1379 && this.var_1140.length > 0)
            {
               _loc3_ = this.var_1140.splice(0,1)[0];
               this.processLoadedSample(_loc3_);
               _loc2_ = getTimer();
            }
            if(this.var_594.length == 0 && this.var_736 != -1)
            {
               this.var_737.ready = true;
               _loc4_ = this.var_736;
               this.var_736 = -1;
               this.var_737 = null;
               events.dispatchEvent(new TraxSongLoadEvent(TraxSongLoadEvent.const_941,_loc4_));
               if(this.var_2300)
               {
      ...
      private function loadNextSong() : void
      {
         var _loc1_:int = 0;
         var _loc2_:* = null;
         if(this.var_737 == null && this.var_1141.length > 0)
         {
            _loc1_ = this.var_1141.getKey(0);
            _loc2_ = this.var_1141.remove(_loc1_);
            if(_loc2_ != null && !_loc2_.disposed)
            {
               this.validateSampleAvailability(_loc2_,true);
               if(_loc2_.ready)
               {
                  events.dispatchEvent(new TraxSongLoadEvent(TraxSongLoadEvent.const_941,_loc1_));
               }
               else
               {
                  this.var_737 = _loc2_;
      ...
      private function processSampleMemoryUsage() : void
      {
         ...
         var _loc1_:* = 0;
         var _loc2_:* = [];
         var _loc3_:Array = this.var_223.samplesIdsInUse;
         var _loc4_:int = 0;
         while(_loc4_ < this.var_250.length)
         {
            _loc5_ = this.var_250.getKey(_loc4_);
            _loc6_ = this.var_250.getWithIndex(_loc4_);
         ...
         if(_loc1_ > const_1380)
         {
            ...
            _loc8_ = 0;
            _loc9_ = 0;
            while(_loc8_ < _loc1_ - const_1378 && _loc9_ < _loc2_.length)
            {
            ...
            if(_loc7_.length > 0)
            {
               for each(_loc11_ in _loc7_)
               {
                  ...
                  _loc12_ = this.var_250.getValue(_loc11_) as TraxSample;
                  _loc12_.dispose();
                  this.var_250.remove(_loc11_);
               }
               this.var_223.samplesUnloaded(_loc7_);
            }
         }
      }
