package com.sulake.habbo.communication.demo
{
   ...
   public class HabboCommunicationDemo extends Component
   {
      ...
      private var var_247:ICoreCommunicationManager;
      ...
      private var var_2747:PseudoRandom;
      ...
      private var var_233:HabboHotelView;
      
      private var var_1563:String = "";
      
      private const const_1963:String = "fuselogin";
      ...
      {
         if(param1 && !this.var_671)
         {
            this.var_671 = param1;
            this.var_57.initConnection(HabboConnectionType.const_244);
         }
      }
      ...
      private function onRoomSessionManagerReady(param1:IID, param2:IUnknown) : void
      {
         ...
         this.var_16.events.addEventListener(RoomSessionEvent.const_97,this.onRoomSessionEnded);
         this.checkRequirements();
      }
      ...
      {
         if(this.var_57 && this.var_144 && this.var_16 && this._windowManager)
         {
      ...
      private function componentsAreRunning() : void
      {
         ...
         var _loc1_:Boolean = this.var_144.getBoolean("client.hotel_view.show_on_startup",true);
         this.var_233 = new HabboHotelView(this._windowManager,assets,Component(context).events,_loc1_);
         var _loc2_:String = this.var_144.getKey("client.hotel_view.image");
         var _loc3_:String = this.var_144.getKey("image.library.url");
         if(_loc2_ != null && _loc3_ != null)
         {
            this.var_233.loadHotelViewImage(_loc3_ + _loc2_);
         }
         var _loc4_:* = this.var_144.getKey("use.sso.ticket") == "1";
         this.var_671 = this.var_144.getKey("sso.ticket",null);
         this.var_1994 = this.var_144.getKey("flash.client.url","");
         this.var_2746 = this.var_144.getKey("external.variables.txt","");
         if(this.var_233 == null)
         {
            this.var_233 = new HabboHotelView(this._windowManager,assets,Component(context).events,_loc1_);
         }
         if(_loc4_)
         {
            this.var_57.mode = HabboConnectionType.const_96;
            if(this.var_671)
            {
               this.var_57.initConnection(HabboConnectionType.const_244);
            }
         ...
         else if(this._windowManager != null)
         {
            ...
            if(_loc5_)
            {
               ...
               this._view.addEventListener(HabboLoginDemoView.const_511,this.onInitConnection);
            }
            else
            {
               Core.crash("Login without an SSO ticket is not supported",Core.const_1707);
            }
         }
      }
      ...
      private function onInitConnection(param1:Event = null) : void
      {
         ...
         {
            this.var_57.mode = HabboConnectionType.const_96;
            if(this.var_144.keyExists("local.environment"))
            {
               this.var_57.mode = HabboConnectionType.const_1280;
            }
         }
         this.var_57.initConnection(HabboConnectionType.const_244);
      }
      ...
      private function onConnectionEstablished(param1:Event = null) : void
      {
         ...
         if(_loc2_ != null)
         {
            this.dispatchLoginStepEvent(HabboCommunicationEvent.const_909);
      ...
      private function onInitCrypto(param1:IMessageEvent) : void
      {
         ...
         this.var_2747 = new PseudoRandom(parseInt(_loc4_.substring(_loc4_.length - 4),16),65536);
         switch(this.var_57.mode)
         {
            case HabboConnectionType.const_96:
               _loc6_ = "";
               _loc6_ = this.var_144.getKey("url.prefix",_loc6_);
               _loc6_ = _loc6_.replace("http://","");
               _loc6_ = _loc6_.replace("https://","");
               _loc7_ = this.var_144.getKey("hotelview.banner.url","http:/\nsitename$/gamedata/banner");
               _loc7_ = _loc7_.replace("$sitename$",_loc6_);
               this.var_1563 = _loc4_;
               this.var_233.loadBannerImage(_loc7_ + "?token=" + this.var_1563,this.onHotelViewBannerLoaded);
               break;
            case HabboConnectionType.const_1280:
               this.sendConnectionParameters(_loc2_);
               break;
            case HabboConnectionType.const_1686:
               this.var_1039 = this.generateRandomHexString(30);
               this.var_543 = new DiffieHellman(new BigInteger(this.var_144.getKey("connection.development.prime"),16),new BigInteger(this.var_144.getKey("connection.development.generator"),16));
               this.var_543.init(this.var_1039);
      ...
      private function onSecretKeyEvent(param1:IMessageEvent) : void
      {
         ...
         var _loc7_:IEncryption = new RC4_R27(_loc6_,this.var_2747);
      ...
      private function sendConnectionParameters(param1:IConnection) : void
      {
         ...
         connection.send(new VersionCheckMessageComposer(401,this.var_1994,this.var_2746));
         var machineId:String = "";
         try
         {
            so = SharedObject.getLocal(this.const_1963,"/");
      ...
      private function onSessionParamsEvent(param1:IMessageEvent) : void
      {
         ...
         if(this.var_671 != null)
         {
            _loc4_ = new SSOTicketMessageComposer(this.var_671);
            _loc2_.send(_loc4_);
      ...
      private function onConnectionDisconnected(param1:Event) : void
      {
         ...
         if(this.var_1564)
         {
            this.dispatchLoginStepEvent(HabboCommunicationEvent.const_193);
         }
         if(param1.type == Event.CLOSE && !this.var_2745)
         {
            _loc2_ = this.var_144.getKey("logout.disconnect.url");
      ...
      private function onDisconnectReason(param1:DisconnectReasonEvent) : void
      {
         ...
         this.var_2745 = true;
         var _loc2_:String = this.var_144.getKey("logout.url");
         if(_loc2_.length > 0)
         {
      ...
      private function setOriginProperty(param1:String) : String
      {
         ...
         {
            return param1.replace("%origin%",this.var_144.getKey("flash.client.origin","popup"));
         }
         return param1;
      }
      ...
      private function onHotelViewBannerLoaded(param1:BitmapData) : void
      {
         ...
         var _loc4_:String = xor(_loc3_,this.var_1563);
