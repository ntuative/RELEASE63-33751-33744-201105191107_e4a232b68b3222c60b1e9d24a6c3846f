package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private var var_710:IFigurePartSet;
      ...
      private var var_579:Rectangle;
      
      private var var_1639:int = 0;
      
      private var var_409:IAvatarRenderManager;
      
      private var var_1084:Boolean;
      
      private var var_1637:BitmapData;
      
      private const const_2201:int = 13421772;
      
      private const const_2202:int = 16777215;
      
      private const const_362:Array = [2,6,0,4,3,1];
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         if(param3 != null)
         {
            ...
            {
               this.var_1639 = Math.max(this.var_1639,_loc6_.colorLayerIndex);
            }
         }
         this.var_409 = this.var_69.controller.avatarRenderManager;
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_702 != null && !this.var_1638)
            {
               _loc3_ = this.var_702;
            }
            else
            ...
            if(this.var_710)
            {
               switch(this.var_710.clubLevel)
               {
                  ...
                  case HabboClubLevelEnum.const_47:
                     _loc2_.style = HabboIconType.const_677;
               }
            }
            else
            {
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         var _loc1_:IAvatarFigureContainer = this.var_409.createFigureContainer(this.partSet.type + "-" + this.partSet.id);
         Logger.log("QUERYING SET:" + this.partSet.type + "-" + this.partSet.id);
         if(!this.var_409.isFigureReady(_loc1_))
         {
            this.var_409.downloadFigure(_loc1_,this);
            return false;
         }
         var _loc6_:int = 0;
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_362[_loc6_] + "_" + FigureData.const_164;
               _loc4_ = this.var_409.getAssetByName(_loc3_) as BitmapDataAsset;
            }
            else
            {
               _loc6_ = 0;
               while(!_loc7_ && _loc6_ < this.const_362.length)
               {
                  _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_362[_loc6_] + "_" + FigureData.const_164;
                  _loc4_ = this.var_409.getAssetByName(_loc3_) as BitmapDataAsset;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         if(this.var_579 == null)
         {
            if(!this.analyzePartLayers())
            {
               if(!this.var_1637)
               {
                  ...
                  this.var_1637 = _loc9_.content as BitmapData;
               }
         ...
         _loc1_ = new BitmapData(this.var_579.width,this.var_579.height,true,16777215);
         var _loc6_:int = 0;
         var _loc7_:Boolean = false;
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               _loc10_ = -1 * _loc4_.offset.x - this.var_579.x;
               _loc11_ = -1 * _loc4_.offset.y - this.var_579.y;
               if(this.var_1638 && _loc2_.colorLayerIndex > 0)
               {
