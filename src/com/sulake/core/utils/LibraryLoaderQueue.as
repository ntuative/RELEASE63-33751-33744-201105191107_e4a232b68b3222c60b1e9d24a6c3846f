package com.sulake.core.utils
{
   ...
   public class LibraryLoaderQueue extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_1888:int = 4;
      ...
      {
         return this.var_426.length + this.var_288.length;
      }
      ...
      private function next() : void
      {
         ...
         if(!disposed)
         {
            while(this.var_288.length < const_1888 && this.var_426.length > 0)
            {
               _loc1_ = this.var_426.shift();
               this.var_288.push(_loc1_);
               _loc1_.resume();
            }
         }
      }
      ...
      private function removeLoader(param1:LibraryLoader) : void
      {
         ...
         try
         {
            index = this.var_426.indexOf(loader);
            if(index > -1)
            {
               this.var_426.splice(index,1);
            }
            index = this.var_288.indexOf(loader);
            if(index > -1)
            {
               this.var_288.splice(index,1);
            }
         }
         catch(e:Error)
         {
      ...
      private function isUrlInQueue(param1:String, param2:Boolean = true) : Boolean
      {
         ...
         if(!disposed)
         {
            ...
            for each(_loc3_ in this.var_426)
            {
               if(param2)
               {
