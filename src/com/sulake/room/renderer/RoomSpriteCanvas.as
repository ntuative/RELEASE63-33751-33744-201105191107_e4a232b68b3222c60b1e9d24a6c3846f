package com.sulake.room.renderer
{
   ...
   public class RoomSpriteCanvas implements IRoomRenderingCanvas
   {
      ...
      private static const const_1532:int = 50;
      
      private static const const_1078:Number = 60;
      
      private static const const_1531:Number = 50;
      ...
      private var var_105:Sprite;
      
      private var var_678:Map;
      
      private var var_549:Point;
      ...
      private var var_1580:IRoomRenderingCanvasMouseListener = null;
      ...
      private var var_2771:Number = -1.0E7;
      
      private var var_2767:Number = -1.0E7;
      ...
      public function RoomSpriteCanvas(param1:IRoomSpriteCanvasContainer, param2:String, param3:int, param4:int, param5:int)
      {
         ...
         this.var_105 = new Sprite();
         this.var_105.name = "canvas";
         this.var_105.mouseEnabled = false;
         this.var_276.addChild(this.var_105);
         this.var_105.mouseEnabled = true;
         this.var_105.doubleClickEnabled = true;
         this.var_105.addEventListener(MouseEvent.CLICK,this.clickHandler);
         this.var_105.addEventListener(MouseEvent.DOUBLE_CLICK,this.clickHandler);
         ...
         this.var_786 = new ColorTransform();
      ...
      {
         this.var_549.x -= param1 - this.var_673;
         this.var_673 = param1;
      }
      
      public function set screenOffsetY(param1:int) : void
      {
         this.var_549.y -= param1 - this.var_674;
         this.var_674 = param1;
      }
      ...
      {
         if(param1 && !this.var_981)
         {
            this.var_981 = true;
            if(this.var_146 != null && !this.var_276.contains(this.var_146))
            {
               this.var_276.addChild(this.var_146);
               this.var_105.mask = this.var_146;
            }
         }
         else if(!param1 && this.var_981)
         {
            this.var_981 = false;
            if(this.var_146 != null && this.var_276.contains(this.var_146))
            {
               this.var_276.removeChild(this.var_146);
               this.var_105.mask = null;
            }
         }
      }
      ...
      public function render(param1:int) : void
      {
         ...
         if(this.var_547 != this.var_2765 || this.var_550 != this.var_2769)
         {
            _loc8_ = true;
         }
         if(this.var_105.x != this.var_673 || this.var_105.y != this.var_674)
         {
            this.var_105.x = this.var_673;
            this.var_105.y = this.var_674;
            _loc8_ = true;
         }
         _loc4_ = 0;
         while(_loc4_ < _loc3_)
         ...
         this.var_2765 = this.var_547;
         this.var_2769 = this.var_550;
      }
      ...
      private function calculateUpdateInterval(param1:int) : void
      {
         ...
         if(this.var_1579 > 0)
         {
            _loc2_ = param1 - this.var_1579;
            if(_loc2_ > const_1078 * 3)
            {
               Logger.log("Really slow frame update " + _loc2_ + "ms");
               this.var_2770 = _loc2_;
            }
            if(_loc2_ <= MAXIMUM_VALID_FRAME_UPDATE_INTERVAL)
            {
               ++this.var_838;
               if(this.var_838 == const_367 + 1)
               {
                  this.var_544 = _loc2_;
                  this.var_2017 = this.var_2768;
               }
               else if(this.var_838 > const_367 + 1)
               {
                  _loc3_ = Number(this.var_838 - const_367);
                  this.var_544 = this.var_544 * (_loc3_ - 1) / _loc3_ + Number(_loc2_) / _loc3_;
                  this.var_2017 = this.var_2017 * (_loc3_ - 1) / _loc3_ + Number(this.var_2768) / _loc3_;
                  if(this.var_838 > const_367 + const_1532)
                  {
                     this.var_838 = const_367;
                     if(!this.var_1284 && this.var_544 > const_1078)
                     {
                        this.var_1284 = true;
                        Logger.log("Room canvas updating really slow - now entering frame skipping mode...");
                     }
                     else if(this.var_1284 && this.var_544 < const_1531)
                     {
                        this.var_1284 = false;
      ...
      private function renderObject(param1:IRoomObject, param2:String, param3:int, param4:Boolean, param5:int) : int
      {
         ...
         _loc6_.update(this._geometry,param3,!_loc9_.isEmpty || param4,this.var_2019 && this.var_1284);
         ...
         while(_loc21_ < _loc12_)
         {
            ...
            if(_loc18_ != null && _loc18_.visible)
            {
               ...
               if(_loc22_ != null)
               {
                  ...
                  _loc20_ = _loc14_ + _loc18_.offsetY + this.var_674;
                  if(this.rectangleVisible(_loc19_,_loc20_,_loc22_.width,_loc22_.height))
                  {
                     ...
                     _loc17_.y = _loc20_ - this.var_674;
      ...
      {
         if(param1 < this.var_547 && param1 + param3 >= 0 && (param2 < this.var_550 && param2 + param4 >= 0))
         {
            if(!this.var_2961)
            {
               return true;
      ...
      private function rectangleVisibleWithExclusion(param1:int, param2:int, param3:int, param4:int) : Boolean
      {
         ...
         if(param1 + param3 >= this.var_547)
         {
            param3 -= this.var_547 + 1 - (param1 + param3);
         }
         if(param2 + param4 >= this.var_550)
         {
            param4 -= this.var_550 + 1 - (param2 + param4);
         }
         for each(_loc5_ in this.var_2766)
         {
      ...
      {
         if(param1 < 0 || param1 >= this.var_1046)
         {
            return null;
         }
         return this.var_105.getChildAt(param1) as ExtendedSprite;
      }
      ...
      private function createSprite(param1:SortableSprite, param2:int = -1) : void
      {
         ...
         if(param2 < 0 || param2 >= this.var_1046)
         {
            this.var_105.addChild(_loc3_);
            ++this.var_1046;
         }
         else
         {
            this.var_105.addChildAt(_loc3_,param2);
         }
         ++this.var_837;
      }
      ...
      private function updateSprite(param1:int, param2:SortableSprite) : Boolean
      {
         ...
         if(_loc4_ != null)
         {
            if(_loc4_.varyingDepth != _loc3_.varyingDepth)
            {
               if(_loc4_.varyingDepth && !_loc3_.varyingDepth)
               {
                  this.var_105.removeChildAt(param1);
      ...
      private function getColoredBitmapData(param1:BitmapData, param2:String, param3:int, param4:Boolean = false) : ExtendedBitmapData
      {
         ...
         if(finalData == null)
         {
            ...
            {
               finalData = new ExtendedBitmapData(data.width,data.height,true,16777215);
               finalData.copyPixels(data,data.rect,const_650);
            }
            catch(e:Error)
            {
               finalData = new ExtendedBitmapData(1,1,true,16777215);
            }
            this.var_786.redMultiplier = tR;
            this.var_786.greenMultiplier = tG;
            this.var_786.blueMultiplier = tB;
            finalData.colorTransform(finalData.rect,this.var_786);
            if(name.length > 0)
            {
               this.var_275.addBitmapData(cacheName,finalData);
            }
      ...
      public function handleMouseEvent(param1:int, param2:int, param3:String, param4:Boolean, param5:Boolean, param6:Boolean, param7:Boolean) : Boolean
      {
         param1 -= this.var_673;
         param2 -= this.var_674;
         this.var_549.x = param1;
         this.var_549.y = param2;
         if(this.var_1283 > 0 && param3 == MouseEvent.MOUSE_MOVE)
         {
      ...
      {
         var _loc11_:Number = param1 - this.var_547 / 2;
            ...
            if(_loc10_ != null && _loc10_.hitTestPoint(param1 - _loc10_.x,param2 - _loc10_.y))
            {
               if(!_loc10_.clickHandling || param3 != MouseEvent.CLICK && param3 != MouseEvent.DOUBLE_CLICK)
               {
                  ...
                  if(_loc12_.indexOf(_loc9_) < 0)
                  {
                     ...
                     if(param3 != MouseEvent.MOUSE_MOVE || param1 != this.var_2771 || param2 != this.var_2767)
                     {
                        this.bufferMouseEvent(_loc11_,_loc9_);
                     }
                     _loc12_.push(_loc9_);
         ...
         while(_loc1_ < this.var_392.length)
         {
            ...
            if(_loc2_ != null && _loc3_ != null)
            {
               ...
               if(_loc4_ != null)
               {
                  ...
                  {
                     this.var_1580.processRoomCanvasMouseEvent(_loc3_,_loc4_,this.geometry);
                  }
                  else
                  {
      ...
      public function update() : void
      {
         if(this.var_1283 == 0)
         {
            this.checkMouseHits(this.var_549.x,this.var_549.y,MouseEvent.MOUSE_MOVE);
         }
         this.var_1283 = 0;
         ++this.var_2018;
      }
